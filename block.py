...
class ConvBlock(nn.Module):
    def __init__(self, dim_in, dim_out, spec_norm=False, LR=0.01, stride=1, up=False):
        ...
        if self.up:
            ...
            #self.up_smaple = nn.ConvTranspose2d(dim_in, dim_out, kernel_size=4, stride=2, padding=1, bias=False)
        else:
            self.up_smaple = None
        ...
        if spec_norm:
            self.main = nn.Sequential(
                ...
                SpectralNorm(nn.Conv2d(dim_out, dim_out, kernel_size=3, stride=stride, padding=1, bias=False)),
                nn.BatchNorm2d(dim_out, affine=True, track_running_stats=True)
            )
            ...
            self.main = nn.Sequential(
                ...
                nn.BatchNorm2d(dim_out, affine=True, track_running_stats=True),
                nn.LeakyReLU(LR, inplace=False),
                nn.Conv2d(dim_out, dim_out, kernel_size=3, stride=stride, padding=1, bias=False),
                nn.BatchNorm2d(dim_out, affine=True, track_running_stats=True),
            )
    ...
    def forward(self, x1, x2=None):
        if self.up_smaple is not None:
            ...
            if x2 is None:
                return self.main(self.up_smaple(x1))
